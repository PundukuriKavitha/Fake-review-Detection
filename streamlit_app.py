...
def main():
    ...
    # Set background
    set_background("background.jpg")  # You can add your background image here
    ...
    if model is None:
        st.error("⚠️ Model not loaded. Please ensure the model files are in the same directory.")
        st.stop()
    ...
    with st.sidebar:
        ...
        with col2:
            st.metric("Model Type", "ML Pipeline")
    ...
    if st.button("🔍 Analyze Text", type="primary", use_container_width=True):
        if user_text.strip():
            with st.spinner("Analyzing text..."):
                ...
                spam_prob = probabilities[1] if len(probabilities) > 1 else confidence
                ...
                with col2:
                    if show_confidence:
                        st.metric(
                            "Confidence",
                            f"{confidence:.1%}",
                            delta=f"{spam_prob:.1%} spam probability"
                        )
